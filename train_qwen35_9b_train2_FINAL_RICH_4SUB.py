...
Final safeguards / improvements:
  ...
  * automatically run test inference after training only for the 0.50 / 1.00 / 1.50 /
    2.00 epoch checkpoints and write submission + probability CSVs.
  * no choice-rotation TTA and no multi-resolution sweep: automatic inference uses only
    the proven 640^2 pixel budget. Training itself also remains at 640.
...
Outputs:
  * checkpoints/epoch_X.XX_update_XXXX/  (all dense checkpoints are still preserved)
  * submissions/r640/submission_epoch_0.50_....csv
  * submissions/r640/submission_epoch_1.00_....csv
  * submissions/r640/submission_epoch_1.50_....csv
  * submissions/r640/submission_epoch_2.00_....csv
  * matching probabilities/r640/*.csv
  * submission_manifest.csv, checkpoints.json, inference_results.json, run_config.json

Automatic inference intentionally runs only 4 times: r640 at epochs 0.50, 1.00, 1.50, 2.00.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    # Automatic submission inference is deliberately minimal: r640 only, and only
    # the four benchmark checkpoint positions 0.50 / 1.00 / 1.50 / 2.00.
    parser.add_argument(
        "--inference-sides",
        type=parse_int_list,
        default=parse_int_list("640"),
        help=(
            "Square pixel-budget side(s) for inference. Default: 640 only. "
            "Leave unchanged for the final run."
        ),
    )
    parser.add_argument(
        "--submission-epochs",
        type=parse_float_list,
        default=parse_float_list("0.50,1.00,1.50,2.00"),
        help=(
            "Checkpoint epochs that receive automatic test inference/submission. "
            "Default: 0.50,1.00,1.50,2.00. Dense LoRA checkpoints are still all saved."
        ),
    )
    ...
    invalid_submission_epochs = [
        ep for ep in args.submission_epochs if ep > args.epochs + 1e-9
    ]
    if invalid_submission_epochs:
        parser.error(
            f"submission epochs cannot exceed --epochs={args.epochs}: "
            f"{invalid_submission_epochs}"
        )

    # Automatic inference can only target checkpoints that are actually saved.
    missing_submission_ckpts = [
        ep for ep in args.submission_epochs
        if not any(abs(ep - ckpt_ep) < 1e-9 for ckpt_ep in args.checkpoint_epochs)
        and abs(ep - args.epochs) >= 1e-9
    ]
    if missing_submission_ckpts:
        parser.error(
            "every --submission-epochs value must also be saved by --checkpoint-epochs: "
            f"{missing_submission_ckpts}"
        )
...
def main() -> None:
    ...
    run_config["submission_epochs"] = list(args.submission_epochs)
    ...
    if args.run_inference:
        ...
        selected_checkpoint_records = [
            ...
            if any(
                abs(float(record.get("requested_epoch", record["current_data_epoch"])) - ep) < 1e-6
                for ep in args.submission_epochs
            )
        ]
        if len(selected_checkpoint_records) != len(args.submission_epochs):
            found = [
                float(record.get("requested_epoch", record["current_data_epoch"]))
                for record in selected_checkpoint_records
            ]
            raise RuntimeError(
                "automatic submission checkpoint selection mismatch: "
                f"requested={list(args.submission_epochs)}, found={found}"
            )

        rich_kv_table(
            "Automatic Submission Plan",
            [
                ("Resolution", "640² pixel budget only"),
                ("Checkpoint epochs", ", ".join(f"{ep:.2f}" for ep in args.submission_epochs)),
                ("Inference runs", str(len(selected_checkpoint_records) * len(args.inference_sides))),
                ("Dense checkpoints saved", str(len(checkpoint_records))),
            ],
            border_style="magenta",
        )
        ...
        # Only the requested benchmark checkpoints receive automatic test inference.
        # All dense LoRA checkpoints remain saved on disk for later manual use.
        for record in selected_checkpoint_records:
        ...
        # No automatic checkpoint ensemble is created in this final fast-inference
        # configuration. The four individual r640 submissions are the intended outputs.
        ...
        if wandb_run is not None:
            ...
            wandb_run.summary["inference/submission_epochs"] = list(args.submission_epochs)
